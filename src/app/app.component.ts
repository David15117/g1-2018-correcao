...
export class AppComponent {
    ...
    pesquisar() {
        for (let i = 0; i < this.ocorrencias.length; i++) {
            if ((this.ocorrencias[i].data >= this.datainicial) && (this.ocorrencias[i].data <= this.datafinal)) {
                this.pesquisaperiodo[i] = this.ocorrencias[i];
                console.log('Entrou');
                console.log(this.pesquisaperiodo[i]);
                this.quantidade++;
